...
这是一个 Windows 免安装包补丁版，重点处理大资料导入和生成知识点覆盖不完整的问题。

### 修复

- 纯文本格式的知识点解析上限从 80 提高到 2000，避免大课件生成结果只保存前半段。
- 新增 PPTX 文字层导入，按幻灯片页码顺序读取课件正文；老版 PPT 会提示另存为 PPTX 或 PDF。
- 更新前端依赖安全补丁，`npm audit` 已无已知漏洞。

## v1.2.3 - 2026-06-26

这是一个 Windows 免安装包补丁版，主要修掉最近几个会卡住正常使用的问题。

### 修复

- 修复 DeepSeek v4 默认 thinking 时，生成计划可能显示“AI 服务器返回了空内容”，但 DeepSeek 侧已经正常计费的问题。
- 修复 Windows 免安装包里 PDF worker 静态资源可能缺失或走到旧资源，导致 PDF 上传解析失败的问题。
- 用户用 DeepSeek API 上传图片解析时，现在会直接提示“DeepSeek 没有多模态，暂时不支持解析图片。”，避免继续发起不支持的图片 OCR 调用。
